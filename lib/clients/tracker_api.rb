module Clients
  class TrackerApi
    ...
    def initialize(options = {}, &block)
      ...
      @token             = options[:token] || "df4b1acf33db3e04e6d8351bc9458fd6"
    ...
    def get(path, options = {})
      request(:get, parse_query_and_convenience_headers(path, options)).body
    end
    ...
    # @return [Array]
    def paginate(path, options = {}, &_block)
      opts           = parse_query_and_convenience_headers path, options.dup
      auto_paginate  = opts[:params].delete(:auto_paginate) { |_k| @auto_paginate }
      @last_response = request :get, opts
      data           = @last_response.body
      raise Clients::Error::UnexpectedData, "Array expected" unless data.is_a? Array
    ...
    def parse_query_and_convenience_headers(path, options)
      ...
      headers       = { "X-TrackerToken" => opts.fetch(:token) }.merge(options.fetch(:headers, {}))
